...
class Toolbar extends Component  {
    ...
    render() {
        ...
        <i style={{margin: "0 5px"}}>|</i> <button onClick={this.props.onShowRegister}>Register</button>
        </div>
        if (this.props.isAuth) {
            ...
            {this.state.dropdown ? 
                <div className={styles.DropdownMenu}>
                    <ul>
                        <li><Link onClick={this.hideDropdown} to="/Orders">Orders</Link></li>
                    </ul>
                </div>
            :null}
             </div>
  
            <i style={{margin: "0 5px"}}>|</i>  <Link to="/Logout"><button>Logout</button></Link>
            </div>
        }
    ...
    return (
        ...
        <div className={styles.UserItems}> 
            ...
            {this.props.loginMode ? <Modal clicked={this.props.onCloseLogin} show={true}>
                <Login clicked={this.props.onSwitchMode} mode={this.props.mode} />
            </Modal> : null}
            {this.props.error ? <Modal show={this.props.error} clicked={this.props.onRemoveError}>
